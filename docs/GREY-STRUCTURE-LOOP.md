...
| I12 | Grid v4: copy/paste + CSV export of filtered rows (T1, T2) | ✅ | browser: CSV blob = 23-col header + 9 rows (RFC-4180 quoted, BOM), ⧉ Copy TSV w/ fallback, multi-line paste into a cell filled the column down (PASTE-A/B verified + restored); fixed heredoc-mangled escapes that briefly broke the page |
