...
| S4 | Shopify product service (dormant till token): product + variants (Full/Deposit), image, unlisted, online store, ≤$500 rule → product-page link | ✅ | ShopifyService: buildProductPayload (title {QuoteID}-{ItemDesc}, vendor EpicCraftings, type, base64 image, active/web, tracked inventory) + variantsFor (Full always, +50% Deposit when >$500, Balance variant); createProduct POSTs REST + returns product-page url; dormant until SHOPIFY_* env. 5 Pest tests green (≤$500 rule, 50/50, balance, inventory, rounding) |
| S5 | "Create payment link" button on the quote → pick Full/Deposit/Balance → creates + stores in ledger | ✅ | browser: proposal shows 💳 block (Full/50% Deposit/Balance for >$500), click captures CLEAN image → POST /quotes/{id}/payment-link. Verified paths: 403 no-permission, 503 "Shopify isn't connected yet" (surfaced in UI), 422 ≤$500-full-only. Live product creation dormant till token; on success stores full ledger snapshot + returns product URL |
